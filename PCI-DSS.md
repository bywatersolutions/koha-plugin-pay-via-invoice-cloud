...
- **Scheduled purge:** As of v1.4.0, a nightly job (Koha's `cronjob_nightly` hook)
...
| Token rows for abandoned checkouts are never purged; `created_on` is recorded but never read. | [`PayViaInvoiceCloud.pm:91-96`][pm91-96] | Affects §5.4. No cardholder data is involved. | **Remediated in v1.4.0** — a nightly job removes tokens older than seven days |
